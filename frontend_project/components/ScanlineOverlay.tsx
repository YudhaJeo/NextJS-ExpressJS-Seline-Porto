...
// Draw a pixel "diamond" star shape
...
// Draw a pixel "snowflake" cross with tips
function drawPixelCross(ctx: CanvasRenderingContext2D, x: number, y: number, size: number) {
  const s = Math.max(1, Math.round(size));
  ctx.fillRect(x - s / 2, y - s * 3, s, s * 6); // vertical
  ctx.fillRect(x - s * 3, y - s / 2, s * 6, s); // horizontal
  // diagonal dots
...
export default function SnowOverlay() {
  ...
  useEffect(() => {
    ...
    // Color palettes: purple/pink/cyan/white
    const PALETTES: [number,number,number][] = [
      [200, 160, 255],  // soft purple
      [157,  78, 221],  // main purple
      [199, 125, 255],  // light purple
      [255, 110, 247],  // pink
      [126, 255, 245],  // cyan
      [230, 200, 255],  // near-white purple
      [255, 220, 255],  // very light pink
    ];

    const kinds: FlakeKind[] = ["pixel", "pixel", "circle", "circle", "star", "cross", "pixel"];

    const FLAKES = 200; // ── lebih rame ──
    ...
    const draw = () => {
      ...
      for (const f of flakes) {
        ...
        switch (f.kind) {
          case "pixel": {
            // Crisp pixel square with optional glow
